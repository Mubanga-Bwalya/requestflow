...
function metaColumns(columns: DataTableColumn[]): DataTableColumn[] {
  return columns.filter((c) => META_KEYS.has(c.key) || c.key === "actionNeeded" || c.key === "nextStep");
}
      ...
      {rows.map((row, i) => (
        ...
        >
          ...
          {actionKey && isValidElement(row[actionKey]) ? (
            <div className="mt-4 flex flex-wrap gap-2 border-t border-zamtel-border pt-3 [&_a]:w-full [&_button]:min-h-11 [&_button]:w-full">
